...
/**
 * Transformer class for processing PostgreSQL change data capture (CDC) records from a streaming source.
 * Converts raw JSON payloads into BigQuery-compatible {@link TableRow} instances.
 *
 * @author david.christianto
 */
final class DataPoolStreamPostgreSqlTransformer {
    ...
    public static TableRow transform(KV<String, String> record, Table table) {
        JsonNode id = JsonUtils.toJsonNode(record.getKey());
        ...
        JsonNode data = isDeleteOperation ? id : payload.get(PAYLOAD_AFTER);
        ...
        return constructTableRow(data, table)
            ...
            .set(LSN, source.get(PAYLOAD_SOURCE_LSN).asLong());
    }
